...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Try WebGL2 first, then WebGL1 with mobile-friendly options
      ...
      if (!gl) {
        console.warn("WebGL not supported, creating fallback");
        ...
        fallback.style.cssText = `
          ...
          background-image: 
            radial-gradient(circle at 25% 25%, ${color}33 2px, transparent 2px),
            radial-gradient(circle at 75% 75%, ${color}22 1px, transparent 1px);
      ...
      const uniforms = {
        ...
        uPixelSize: {
          value:
            (isMobile ? Math.max(pixelSize * 1.5, 6) : pixelSize) *
      ...
      const animate = (currentTime: number = 0) => {
        ...
        uniforms.uTime.value =
          timeOffset +
          clock.getElapsedTime() * speedRef.current * (isMobile ? 0.8 : 1);
        if (liquidEffect && "uniforms" in liquidEffect) {
          const timeUniform = (
            liquidEffect as { uniforms: Map<string, THREE.Uniform> }
          ).uniforms.get("uTime");
      ...
      if (t.liquidEffect && "uniforms" in t.liquidEffect) {
        const liquidUniforms = (
          t.liquidEffect as { uniforms: Map<string, THREE.Uniform> }
        ).uniforms;
        const uStrength = liquidUniforms.get("uStrength");
        if (uStrength) uStrength.value = liquidStrength;
        const uFreq = liquidUniforms.get("uFreq");
