...
#define HFOG_EDGE_START 0.0
#define HFOG_EDGE_END 1.0
...
export const LaserFlow: React.FC<Props> = ({
  ...
  const uniformsRef = useRef<{ [uniform: string]: THREE.IUniform } | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    canvas.addEventListener("pointerdown", onMove as EventListener, {
      passive: true,
    });
    canvas.addEventListener("pointerenter", onMove as EventListener, {
      passive: true,
    });
    canvas.addEventListener("pointerleave", onLeave as EventListener, {
      passive: true,
    });
